...
<# Using Test Cases (Shorter)
Describe 'My Solution' {
  It "Given '<Input>', it returns '<Expected>'" -TestCases @(
    @{ Input = @(16, 11, 5, 3); Expected = 96 }
    @{ Input = @(1, 1, 1, 1); Expected = 0 }
    @{ Input = @(13, 6, 8, 3); Expected = 18 }
    @{ Input = @(60, 100, 60, 1); Expected = 99 }
    @{ Input = @(1000, 1000, 1000, 1000); Expected = 0 }
  ) {
